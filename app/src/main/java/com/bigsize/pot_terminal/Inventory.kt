...
class Inventory:DensoWaveBase() {
  ...
  fun execEnter():Boolean {
    ...
    // 確定した数量を累計します
    viewModel01.countUPAmt( "01", viewModel01.edtAmt.value!! )
    viewModel01.countUPAmt( "02", viewModel01.edtAmt.value!! )
   ...
   * 入力チェックを行います
   *
   * @param [execSubject] チェックモード 01 : 棚み取り時 02 : 箱読み取り時 03 : 商品読み取り時 04 : ENT押下時
   * @return 入力チェック結果
   */
  ...
  private fun execDataSave() {
    ...
    if( viewModel01.edtAmt.value != "" ) { amt = viewModel01.edtAmt.value!! }
